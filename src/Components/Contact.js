  ...
  return contacts.map((contact) => {
    return (
      <a href={contact.link} key={contact.source} target="_blank" rel="noreferrer">
        <div className="flex items-center px-12 gap-6 rounded-xl bg-links p-6 anim-hover-scale-110">
...
const Contact = () => {
  return (
    <div className="flex flex-col items-center gap-10 text-accent">
      ...
      <div className="flex flex-col md:flex-row gap-6">{renderContacts()}</div>
    </div>
  );
};
